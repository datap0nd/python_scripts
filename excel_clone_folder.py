...
Opens each .xlsx via COM, reads data from memory, clones using:
  Approach 1: Duplicate unzipped template → populate XMLs → rezip
  Approach 2: Read via COM + rebuild with openpyxl (fallback)

Saves cloned files to a "new" subfolder inside the source folder.
...
def main():
    ...
    # Find all .xlsx files in that folder (exclude "new" subfolder)
    xlsx_files = [
        f for f in glob.glob(os.path.join(folder, "*.xlsx"))
        if not f.startswith(os.path.join(folder, "new"))
    ]
    if not xlsx_files:
    ...
    for filepath in xlsx_files:
        ...
        try:
            # Open in Excel via COM (data is decrypted in memory)
